...
$city_edit = NULL; // Initialize page object first

class ccity_edit extends ccity {
	...
	function ValidateForm() {
		...
		if (!ew_CheckInteger($this->province->FormValue)) {
			ew_AddMessage($gsFormError, $this->province->FldErrMsg());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// province
			$this->province->SetDbValueDef($rsnew, $this->province->CurrentValue, 0, $this->province->ReadOnly);
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "citylist.php", $this->TableVar, TRUE);
		$PageId = "edit";
...
// Create page object
if (!isset($city_edit)) $city_edit = new ccity_edit();
...
// Form object
var fcityedit = new ew_Form("fcityedit");
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($city->province->FldErrMsg()) ?>");
...
fcityedit.ValidateRequired = true;
<?php } else { ?>
fcityedit.ValidateRequired = false; 
<?php } ?>
...
?>
<form name="fcityedit" id="fcityedit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="city">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_cityedit" class="table table-bordered table-striped">
<?php if ($city->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_city_id"><?php echo $city->id->FldCaption() ?></span></td>
		<td<?php echo $city->id->CellAttributes() ?>>
<span id="el_city_id" class="control-group">
<span<?php echo $city->id->ViewAttributes() ?>>
<?php echo $city->id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($city->id->CurrentValue) ?>">
<?php echo $city->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<span id="el_city_name" class="control-group">
<input type="text" data-field="x_name" name="x_name" id="x_name" size="30" maxlength="64" placeholder="<?php echo ew_HtmlEncode($city->name->PlaceHolder) ?>" value="<?php echo $city->name->EditValue ?>"<?php echo $city->name->EditAttributes() ?>>
</span>
<?php echo $city->name->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<span id="el_city_province" class="control-group">
<input type="text" data-field="x_province" name="x_province" id="x_province" size="30" placeholder="<?php echo ew_HtmlEncode($city->province->PlaceHolder) ?>" value="<?php echo $city->province->EditValue ?>"<?php echo $city->province->EditAttributes() ?>>
</span>
<?php echo $city->province->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
