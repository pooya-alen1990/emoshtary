...
class cimages extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('images', 'images', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// original_image
		$this->original_image = new cField('images', 'images', 'x_original_image', 'original_image', '`original_image`', '`original_image`', 200, -1, FALSE, '`original_image`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['original_image'] = &$this->original_image;

		// thumbnail_image
		$this->thumbnail_image = new cField('images', 'images', 'x_thumbnail_image', 'thumbnail_image', '`thumbnail_image`', '`thumbnail_image`', 200, -1, FALSE, '`thumbnail_image`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['thumbnail_image'] = &$this->thumbnail_image;

		// ip_address
		$this->ip_address = new cField('images', 'images', 'x_ip_address', 'ip_address', '`ip_address`', '`ip_address`', 200, -1, FALSE, '`ip_address`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ip_address'] = &$this->ip_address;

		// advertise_id
		$this->advertise_id = new cField('images', 'images', 'x_advertise_id', 'advertise_id', '`advertise_id`', '`advertise_id`', 3, -1, FALSE, '`advertise_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->advertise_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['advertise_id'] = &$this->advertise_id;
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->original_image->Exportable) $Doc->ExportCaption($this->original_image);
				if ($this->thumbnail_image->Exportable) $Doc->ExportCaption($this->thumbnail_image);
				if ($this->ip_address->Exportable) $Doc->ExportCaption($this->ip_address);
				if ($this->advertise_id->Exportable) $Doc->ExportCaption($this->advertise_id);
			} else {
				if ($this->id->Exportable) $Doc->ExportCaption($this->id);
				if ($this->original_image->Exportable) $Doc->ExportCaption($this->original_image);
				if ($this->thumbnail_image->Exportable) $Doc->ExportCaption($this->thumbnail_image);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->original_image->Exportable) $Doc->ExportField($this->original_image);
					if ($this->thumbnail_image->Exportable) $Doc->ExportField($this->thumbnail_image);
					if ($this->ip_address->Exportable) $Doc->ExportField($this->ip_address);
					if ($this->advertise_id->Exportable) $Doc->ExportField($this->advertise_id);
				} else {
					if ($this->id->Exportable) $Doc->ExportField($this->id);
					if ($this->original_image->Exportable) $Doc->ExportField($this->original_image);
					if ($this->thumbnail_image->Exportable) $Doc->ExportField($this->thumbnail_image);
